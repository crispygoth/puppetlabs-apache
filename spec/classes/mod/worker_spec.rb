...
describe 'apache::mod::worker', :type => :class do
  ...
  context "on a Debian OS" do
    ...
    context "with Apache version >= 2.4 on Debian 9" do
      ...
      it { is_expected.to contain_file("/etc/apache2/mods-available/mpm_worker.load").with({
        'ensure'  => 'file',
        'content' => "LoadModule mpm_worker_module /usr/lib/apache2/modules/mod_mpm_worker.so\n"
        })
      }
      it { is_expected.to contain_file("/etc/apache2/mods-enabled/mpm_worker.load").with_ensure('link') }
    end
  end
